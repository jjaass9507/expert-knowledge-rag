...
- `/submit` 貼上逐字稿或上傳音檔 → 多段 LLM 自動結構化（結構化 → 萃取重點 → 濃縮內容）
- `/` 待校稿清單 → 進入校稿台逐欄校對（含重點、設備大分類）→「確認」/「退回重整理」
- `/library` 知識卡目錄 → 瀏覽、**編輯、刪除**已定稿卡片
- `/search` 知識檢索 → 查詢知識庫（開向量化用語意檢索，未開則關鍵字搜尋）
- 核准卡片寫入 `data/approved/cards.jsonl`（向量化來源）與 `data/approved/yaml/<id>.yaml`（git 溯源）

### 知識卡欄位重點
- **重點**：2-5 條速查條列，由內容萃取（與「內容」全文敘述互補）
- **設備大分類**：空壓機/冰水主機/泵浦/… 預設清單（見 `EQUIPMENT_CATEGORIES`，可調整），供分類與檢索過濾
