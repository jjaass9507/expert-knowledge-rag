...
def _strip_think(text: str) -> str:
    """移除推理模型（如 qwen3）夾帶的 <think>…</think> 推理段，只留真正答案。

    若答案落在未配對的結尾 </think> 之後（模型省略開頭標籤），取最後一個 </think> 之後的內容。
    """
...
def _call_json(llm: LLM, system: str, human: str, max_retries: int = 1) -> dict:
    ...
    for attempt in range(max_retries + 1):
        ...
        if isinstance(data, dict):
            return data
        current = human + "\n\n請只輸出合法 JSON 物件。"
    log.warning("附加 pass 無法解析為 JSON 物件，原始輸出：%s", _snippet(raw))
...
def complete_card(card: KnowledgeCard, llm: LLM, max_retries: int = 1) -> KnowledgeCard:
    ...
    try:
        ...
        log.warning("補全 pass 失敗，保留原卡（標題：%s）：%s", card.標題, e)
        return card
...
def _extract_knowledge_units(transcript: str, llm: LLM, max_retries: int = 1) -> list[str]:
    ...
    try:
        ...
        log.warning("階段一萃取知識單元失敗，改走直接結構化後備：%s", e)
        return []
